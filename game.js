...
var female_pacman = {image: img_female_pacman, directionX: 13, directionY: 13};
...
function newGameBtnClicked() {
    let r = confirm("Are you sure you want to start a new game? Notice that your will reset.");
    if (r == true) {
...
function initBoard() {
    ...
    max_points = prem_food_remain * 25 + solid_food_remain * 15 + junk_food_remain * 5 + 50;
    var pacman_remain = 1;
...
function setFemalePacmanInitPosition() {
    female_pacman.directionX = 13;
    female_pacman.directionY = 13;
}
...
function startNewGame() {
    ...
    initBoard();
    initNewRound()
    document.getElementById("music_video").play();
...
function drawLives() {
    let src = document.getElementById("lives");
    while (src.firstChild) {
        src.removeChild(src.firstChild);
    }
    for (var i = 0; i < pacman_lives; i++) {
        let img = document.createElement("img");
        img.src = "files/mis-pacman.jpg";
        let src = document.getElementById("lives");
        src.appendChild(img);
    }
}
...
function drawGameOver() {
    let src = document.getElementById("lives");
    let gameOver = document.createElement("label");
    gameOver.text = "Game Over";
    src.appendChild(gameOver);
}
...
function UpdatePosition() {
    ...
    if (score + (3 - pacman_lives) * 10 == max_points) {
        window.alert("We have a supper winner!!!! good job!");
        window.clearInterval(interval);
        window.clearInterval(interval_ghost);
        return;
    }
    ...
    if (pacmanMeetsGhost()) {
        ...
        if (pacman_lives == 1 && pacmanMeetsGhost) {
            ...
            drawGameOver();
            window.clearInterval(interval);
            window.alert("You Lost!, You don't have anymore live, you can reset the game");
            muteMusic();
            return;
        } else {
            pacman_lives--;
